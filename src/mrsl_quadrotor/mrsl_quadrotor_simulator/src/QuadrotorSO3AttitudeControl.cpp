...
void QuadrotorSO3AttitudeControl::init(const Quadrotor &quad, float I[][3]) {
  ...
  std::cout << "km :" << km_ << std::endl;
  std::cout << "al :" << arm_length_ << std::endl;
}

void QuadrotorSO3AttitudeControl::cmdCallback(
    const QuadrotorSO3AttitudeControl::CmdMsg::ConstPtr &msg) {
  cmd = *msg;
}

Quadrotor::MotorState
QuadrotorSO3AttitudeControl::getControl(const Quadrotor::State &state) {
  if (!cmd)
  ...
  Eigen::Quaterniond q_des(cmd->orientation.w, cmd->orientation.x,
                           cmd->orientation.y, cmd->orientation.z);
  ...
  // const float Omd1 = des_angvel_x * (R11 * Rd11 + R21 * Rd21 + R31 * Rd31) +
  //                    des_angvel_y * (R11 * Rd12 + R21 * Rd22 + R31 * Rd32) +
  //                    des_angvel_z * (R11 * Rd13 + R21 * Rd23 + R31 * Rd33);
  // const float Omd2 = des_angvel_x * (R12 * Rd11 + R22 * Rd21 + R32 * Rd31) +
  //                    des_angvel_y * (R12 * Rd12 + R22 * Rd22 + R32 * Rd32) +
  //                    des_angvel_z * (R12 * Rd13 + R22 * Rd23 + R32 * Rd33);
  // const float Omd3 = des_angvel_x * (R13 * Rd11 + R23 * Rd21 + R33 * Rd31) +
  //                    des_angvel_y * (R13 * Rd12 + R23 * Rd22 + R33 * Rd32) +
  //                    des_angvel_z * (R13 * Rd13 + R23 * Rd23 + R33 * Rd33);

  const float Omd1 = -eR1 * 6.0f;
  const float Omd2 = -eR2 * 6.0f;
  const float Omd3 = -eR3 * 3.0f;
